...
public class UsuarioService {
    ...
    private Usuario foundUsuario(Long id){
        return this.usuarioRepository.findById(id)
                .orElseThrow(() -> new RuntimeException(String.format(String.format("Usuário '%d' não encontrado", id))));
    }

    private Usuario foundLogin(String login){
        return this.usuarioRepository.findByLogin(login)
                .orElseThrow(() -> new RuntimeException(String.format(String.format("Login '%s' não encontrado", login))));
    }
